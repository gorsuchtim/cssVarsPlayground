...
function appendShape() {
  ...
  document.querySelector(".shapeContainer").innerHTML +=
    '<div class="shape" id="shape' + totalShapes + '"></div>'; // add the shape to the container
  document.getElementById("selectorExisting").innerHTML +=
    '<option class="selector__existingOption" value="shape' +
    totalShapes +
    '" id="dropdown' +
    totalShapes +
    '">Shape ' +
...
yesDeleteButton.addEventListener("click", function() {
  // here we need to know if remove or clear is selected and we pass it to delete shape//test if confirmdelete !hasclass hidden and which button is flagged/selected
  var confirmDeleteContent = document.querySelectorAll(".confirmDeleteContent");
  for (var i = 0; i < confirmDeleteContent.length; i++) {
    toggleConfirmContent();
  }
...
function deleteShape() {
  if (flaggedButton === "removeShape") {
    ...
    if (document.querySelector(".selector__existing").firstElementChild) {
      // if dropdown has a shape in it
      document.querySelector(
        ".selector__existing"
      ).firstElementChild.selected = true; // set dropdown to first shape in it
    }
    ...
    document.querySelector(".selector__existing").innerHTML +=
      '<option class="selector__existingOption" value="shape1" id="dropdown1">Shape 1</option>';
    // JQUERY UI to make shapes draggable
    $(".shape").draggable();
  }
...
var borderCheckbox = document.querySelectorAll(
  ".inputBox__checkbox--borderCheckbox"
);
